...
boot_alloc(uint32_t n)
{
	...
	// PGSIZE - is the size of page in bytes. 
	if (npages * PGSIZE <= PADDR(ROUNDUP((char *)(nextfree + n), PGSIZE))) { 
		panic("boot_alloc: out of memory\n"); 
	} 

	if (n > 0) { 
		char *nextfree_before_alloc = nextfree; 
		nextfree = ROUNDUP((char *) nextfree + n, PGSIZE); 
		return nextfree_before_alloc; 
	} 
...
page_init(void)
{
	...
	size_t i;
	for (i = 1; i < npages; i++) {
		physaddr_t ph_addr = page2pa(&pages[i]);
		
		if ((ph_addr >= IOPHYSMEM && ph_addr < EXTPHYSMEM) || // in IO hole 
			(ph_addr >= EXTPHYSMEM && page2kva(&pages[i]) < boot_alloc(0))) { // in extended physical memory
			pages[i].pp_ref = 1; 
			continue;
		}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	static uintptr_t base = MMIOBASE; base=base;
